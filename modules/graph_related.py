...
def likelihood_triplet(G,X,Y,new_mu,new_cov,new_cov_sqrt,z,z_weights):
    ...
    if tf.shape(X)[0] == 0 or tf.shape(Y)[0]==0:
        return tf.zeros([1]), tf.zeros([G.p]), tf.zeros([G.p,G.p])
    ...
    phi, grad_phi, hess_phi = activated_functions(activations,Y,G.Phi,G.grad_Phi,G.hess_Phi)
    ...
    computed_rho_l  = tf.einsum('npk,nk->p', A_array, local_r,name="computed_rho_l") #[p]=[n,p,k]×[n,k]
   
    if G.compute_kernel:
        computed_beta = tf.einsum('npkqj,nkj->pq', A_array_kernel, local_B, name="computed_beta_l") #[p,p]=[n,p,k,p,k]×[n,p,k]
    else:
        #computed_beta = tf.einsum('ijk,ikl,iml->jm', A_array, local_B, A_array)
        computed_beta_aux = tf.einsum('ijk,ikl->ijl', A_array, local_B)
        computed_beta = tf.einsum('ijl,iml->jm',computed_beta_aux, A_array, name = "computed_beta_l")
...
def prior_triplet(G,prior_std,new_mu,new_cov,z_prior,z_weights_prior):
    ...
    if not G.keep_1d_prior:
        ...
        if G.compute_prior_kernel:
            ...
            local_std_prior = tf.sqrt(tf.einsum('ijk,jk->i', hess_multipliers, new_cov)) # [p]
        else:
            local_std_prior = tf.sqrt(tf.einsum('ij,jk,ik->i', multipliers, new_cov, multipliers))
    
    else:
        multipliers = 1/prior_std
        hess_multipliers = tf.square(multipliers)
        local_mu_prior = tf.einsum('i,i->i', multipliers, new_mu)
        local_std_prior = tf.sqrt(tf.einsum('i,i->i',hess_multipliers, tf.diag_part(new_cov)))
    ...
    if not G.keep_1d_prior:
        ...
        computed_beta_prior = tf.diag(tf.einsum('i,i->i', mean_phi_hessian_prior, hess_multipliers))
...
def batched_prior_triplet(G,prior_std,*pars):
    ...
    Computed_e_prior, Computed_rho_prior, Computed_beta_prior =\
        tf.map_fn(lambda x : prior_triplet(G,x,*pars), prior_std_, dtype=(tf.float32, tf.float32, tf.float32))
